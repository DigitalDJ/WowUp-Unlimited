...
export class OptionsAddonSectionComponent implements OnInit, OnDestroy {
      ...
      .pipe(
        ...
        switchMap((ch) => {
          ...
          if (typeof ch?.cfV2ApiKey === "string") {
            tasks.push(
              from(this._sensitiveStorageService.setAsync(PREF_CF2_API_KEY, ch.cfV2ApiKey))
            );
          }
  ...
  private async loadSensitiveData() {
    try {
      const cf2ApiKey = await this._sensitiveStorageService.getAsync(PREF_CF2_API_KEY);
      ...
      this.preferenceForm.get("cfV2ApiKey")?.setValue(cf2ApiKey);
